...
namespace StackAnalyzer
{
	class Program
	{
		...
		private static void PopulateUsersLocation(int skip = 0)
		{
			...
			try
			{
				...
				foreach (dynamic user in usersWithLocation)
				{
					...
					_logger.Info($"Processing user {i++}, user id: {userId}, location: {locationString}");
					...
					var proxyUri = "http://137.74.195.38:3128";
					...
					if (geoResult["status"] != "OK")
					{
						...
						{
							AddQueryLimitError(queryOverLimit, user);
						}
		...
		private static void AddQueryLimitError(List<dynamic> queryOverLimit, dynamic user)
		{
			if (File.Exists(QueryOverLimitFile))
			{
				queryOverLimit = JsonConvert.DeserializeObject<List<dynamic>>(File.ReadAllText(LocationsFile));
			}
